...
Home location is set by dragging a pin on an OpenStreetMap map — no Google account
or API key needed.
...
4. First launch: grant location (choose "While using the app" first), then use
   the in-app prompt to upgrade to "Allow all the time" — required for geofencing
   to work when the app isn't open — and allow notifications.
5. For reliability, also do what the in-app tip on the Settings screen says:
...
- Default geofence radius: 150 m, adjustable 50–500 m from the Settings screen.
